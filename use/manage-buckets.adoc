...
NOTE: Al añadir un bucket, selecciona el proveedor de buckets adecuado e introduce las credenciales correctas para ese proveedor. Por ejemplo, la interfaz de usuario acepta NetApp ONTAP S3 como tipo y acepta credenciales de StorageGRID; sin embargo, esto hará que todas las futuras copias de seguridad y restauraciones de apps que usen este bucket fallen. Consulta el link:../release-notes/known-issues.html["Notas de la versión"].
