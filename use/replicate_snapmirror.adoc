...
* Los clústeres de ONTAP y la SVM de host se deben emparejar. Consulte https://["Información general sobre relaciones entre iguales de clústeres y SVM"^].
...
* Las licencias asíncronas de SnapMirror de ONTAP que utilizan el paquete de protección de datos deben habilitarse en los clústeres de ONTAP de origen y de destino. Consulte https://["Información general sobre las licencias de SnapMirror en ONTAP"^].
* Al añadir un back-end de almacenamiento de ONTAP a Astra Control Center, aplique las credenciales de usuario con la función "admin", que cuenta con métodos de acceso `http` y.. `ontapi` Se habilitó en los clústeres de origen y destino de ONTAP. Consulte https://["Gestionar cuentas de usuario en la documentación de ONTAP"^] si quiere más información.
