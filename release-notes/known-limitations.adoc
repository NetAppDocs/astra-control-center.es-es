...
Si quieres gestionar un clúster en otra instancia de Astra Control Center, primero debes link:../use/unmanage.html#unmanage-a-cluster["anule la gestión del clúster"] eliminarlo de la instancia en la que se gestiona antes de gestionarlo en otra instancia. Después de quitar el clúster de la gestión, verifica que el clúster ya no esté gestionado ejecutando este comando:
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["Instala la versión más reciente de Astra Trident"^].
...
Asegúrate de link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["introduzca los valores correctos"] al establecer una conexión.
