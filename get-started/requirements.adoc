...
+
** El token del archivo de configuración predeterminado de VMware TKG y TKGi caduca diez horas después de la implementación. Si usas productos del portafolio de Tanzu, debes generar un archivo de configuración de Tanzu Kubernetes Cluster con un token sin caducidad para evitar problemas de conexión entre Astra Control Center y los clústeres de aplicaciones gestionados. Para obtener instrucciones, visita  https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-2.html["la documentación del producto VMware NSX-T Data Center"^].
